...
function kiasu_bc.encrypt(key, tweak, plaintext)
    ...
    for i = 1, 16 do
        state[i] = bxor(bxor(state[i], round_keys[i]), padded_tweak_state[i])
    end
    
    -- Main rounds
    for round = 1, 9 do
        state = aes_core.sub_bytes(state)
        state = aes_core.shift_rows_row_major(state)
        state = aes_core.mix_columns(state)
        
        for i = 1, 16 do
            state[i] = bxor(bxor(state[i], round_keys[round * 16 + i]), padded_tweak_state[i])
        end
    end
    ...
    for i = 1, 16 do
        state[i] = bxor(bxor(state[i], round_keys[160 + i]), padded_tweak_state[i])
    end
...
function kiasu_bc.decrypt(key, tweak, ciphertext)
    ...
    state = aes_core.inv_sub_bytes(state)
    ...
    for round = 9, 1, -1 do
        ...
        state = aes_core.inv_mix_columns(state)
        state = aes_core.inv_shift_rows_row_major(state)
        state = aes_core.inv_sub_bytes(state)
    end
